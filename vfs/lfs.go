...
type LogStructuredFS struct {
	...
	backgroundSyncWorker *time.Ticker
}
...
func (lfs *LogStructuredFS) StopBackgroundSync() {
	lfs.mu.Lock()
	defer lfs.mu.Unlock()

	if lfs.backgroundSyncWorker != nil {
		lfs.backgroundSyncWorker.Stop()
	}
}
...
func (lfs *LogStructuredFS) backgroundSync() {
	for range lfs.backgroundSyncWorker.C {
...
func (lfs *LogStructuredFS) changeRegions() error {
	...
	lfs.regions[lfs.regionID].ReaderAt = reader
	...
	for _, file := range files {
		if !file.IsDir() && strings.HasSuffix(file.Name(), fileExtension) {
			if strings.HasPrefix(file.Name(), "0") {
				...
				reader, err := mmap.Open(filepath.Join(lfs.directory, file.Name()))
				if err != nil {
					return fmt.Errorf("failed to mmap data file: %w", err)
				}
				...
				lfs.regions[regionID] = &Region{Fd: fd, ReaderAt: reader}
			}
		}
	}
...
func OpenFS(opt *Options) (*LogStructuredFS, error) {
	...
	instance := &LogStructuredFS{
		...
		backgroundSyncWorker: time.NewTicker(time.Duration(3) * time.Second),
	}
...
func (lfs *LogStructuredFS) CloseFS() error {
	lfs.mu.Lock()
	defer lfs.mu.Unlock()
	for _, reg := range lfs.regions {
		if reg.ReaderAt != nil {
			err := reg.ReaderAt.Close()
			if err != nil {
				return fmt.Errorf("failed to close mmap reader: %w", err)
			}
		}
		err := utils.FlushToDisk(reg.Fd)
		if err != nil {
...
func scanAndRecoverCheckpoint(files []string, regions map[int64]*Region, indexs []*indexMap) error {
	var (
		ckpt    int
		path    string
		pauseID string
